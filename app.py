...
def extract_sections(text):
    ...
    sections["title"] = re.search(
        r"(?<=Title:)(.*?)(?=Abstract:)", text, re.DOTALL).group().strip()
    sections["abstract"] = re.search(
        r"(?<=Abstract:)(.*?)(?=Background:)", text, re.DOTALL).group().strip()
    sections["background"] = re.search(
        r"(?<=Background:)(.*?)(?=Summary:)", text, re.DOTALL).group().strip()
    sections["summary"] = re.search(
        r"(?<=Summary:)(.*?)(?=Detailed Description:)", text, re.DOTALL).group().strip()
    sections["description"] = re.search(
        r"(?<=Detailed Description:)(.*?)(?=Claims:)", text, re.DOTALL).group().strip()
    sections["claims"] = re.search(
        r"(?<=Claims:)(.*)", text, re.DOTALL).group().strip()
...
st.title("Patent Summarization and Explanation")
...
if uploaded_file is not None:
    ...
    background_summary = get_gpt3_response(
        background_summary_prompt, max_tokens=100)
    ...
    first_claim_explanation = get_gpt3_response(
        claim_explanation_prompt, max_tokens=100)
